...
const  [rangeStart, rangeEnd] = [toLocaleDateString(addDays(todayDateString, 3)), toLocaleDateString(addDays(todayDateString, 7))]
...
const PaginatedCalendarListComponent = ({ locale }) => {
    ...
    const { markedDates, onDayPress, maxDate } = useMultiSelectCalendar(30, rangeStart, rangeEnd);
    ...
    return (
        ...
        />
            {loading && <ActivityIndicator size="large" style={{marginHorizontal: 'auto'}} />}
        </>
    );
};
